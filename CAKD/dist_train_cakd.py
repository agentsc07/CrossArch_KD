"""
CIFAR-10 Adapted ResNet50 + ViT-B/16 Cross-Architecture Knowledge Distillation (CAKD) Training Script
Modified from: dist_train_cakd.py (original ImageNet version)
...
- Updated output directory to ./results/cifar10_cakd_vitb16
- Removed DistributedDataParallel wrapping (commented out)
- Simplified data loading and evaluation
- Kept all three loss components: PCA loss, GL loss, GAN loss
- Teacher model (ViT-B/16) and Discriminator used in training

METHOD: Full CAKD with:
  1. Partial Cross Attention projector (PCA loss)
  2. Group-wise Linear projector (GL loss)
  3. Adversarial training with discriminator (GAN loss)

For full changelog, see: ../CHANGELOG.md

Run: python dist_train_cakd.py --epochs 30 --batch-size 64 --model-ema --device cuda
Or: bash experiments/run_cakd_cifar10.sh
"""
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--output-dir", default="./results/cifar10_cakd", type=str, help="path to save outputs"))
